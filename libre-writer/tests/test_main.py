...
# Test correct office path is returned for Windows
...
# Test correct office path is returned for Linux
...
# Check that an error is returned if file path is not found
...
# Check that an error is returned for an unsupported OS
def test_get_office_path_unsupported_os(monkeypatch):
    monkeypatch.setattr("platform.system", lambda: "Darwin")
    with pytest.raises(OSError):
        get_office_path()

# Check the correct python path is returned on Windows
...
# Check that fallback python path is used correctly
...
# Check correct python path is returned for Linux
...
def test_is_port_in_use_true_false(monkeypatch):
    ...
    class DummySocket:
        ...
        def connect_ex(self, addr) -> int: return 0
    ...
    class DummySocket2(DummySocket):
        def connect_ex(self, addr) -> int: return 1
...
def test_main_exception(monkeypatch):
    monkeypatch.setattr("main.start_office", lambda: (_ for _ in ()).throw(Exception("fail")))
    exit_mock = mock.Mock()
